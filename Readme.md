...
##Executive Summary
This repository details an end-to-end anlytical pipeline designed to and explain customer churn within retail banking environment. This project will priotize model transparency and actionable insights bt integrating hyperparameter tuning with tools like optuna and shapley addictive explanations (SHAP) for feature attribution.
...
To ensure peak performanceon the imbalanced data, I utilized smote to creaate synthectic churners thus making the model not only to learn on majority classes(Existing customers) but also churning ones and then used tree-structured parzen estimator via optuna for parameter tuning:
Objective: Maximize fi-score to blance precision and recall.
Outcome: An automated 30-trial study converged on an optimal configuration for XGboost thus significally increasing its performance by utilizing optimized parameters. 
...
*Local Explainability: Waterfall plot will provide a particulat customer reasons for churning thus allowing for a personalized retention offers
